"""
Tests for src/win_probability_engine.py (ported from
project_hrishav/win_probability.py).
"""
import numpy as np
import torch
...
def test_mc_dropout_produces_nonzero_variance():
    """With dropout active across multiple stochastic passes, std should be > 0
    for at least some positions (dropout must not have been silently disabled)."""
    torch.manual_seed(0)
    model = IPLTransformer()
...
def test_model_returns_to_eval_mode_after_predict():
    """predict() must leave the model in eval mode afterward (no lingering
    dropout during normal inference elsewhere)."""
    torch.manual_seed(0)
    model = IPLTransformer()
...
def test_update_matches_last_element_of_predict():
    ...
    torch.manual_seed(1)  # reset seed so update()'s internal predict() is comparable
