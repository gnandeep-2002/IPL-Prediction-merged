...
def _classification_row(r: Report, m: dict) -> None:
    """Continuation output lines per classifier: log loss + threshold-0.5
    metrics (positive class = whatever the model's probability is expressed
    in). Rec = sensitivity/TPR; Spec = TNR; FPR/FNR are their complements."""
    cm = m["confusion"]
    r.row("", LogLoss=m["log_loss"], Prec=m["precision"], Rec=m["recall"],
          Spec=m["specificity"], F1=m["f1"])
    r.row("", FPR=m["fpr"], FNR=m["fnr"],
...
def run_pre_match_section(r: Report, match_df: pd.DataFrame) -> dict:
    ...
    _classification_row(r, pre["climatology"])
    r.row("Cal. LR", Brier=pre["cal_lr"]["brier"], AUC=pre["cal_lr"]["auc"],
          Acc=pre["cal_lr"]["acc"], BSS=pre["cal_lr"]["bss"], ECE=pre["cal_lr"]["ece"])
    _classification_row(r, pre["cal_lr"])
    r.row("Cal. GBT", Brier=pre["cal_gbt"]["brier"], AUC=pre["cal_gbt"]["auc"],
          Acc=pre["cal_gbt"]["acc"], BSS=pre["cal_gbt"]["bss"], ECE=pre["cal_gbt"]["ece"])
    _classification_row(r, pre["cal_gbt"])
    r.note("positive class throughout section 1: team1 (the bat-first side) "
           "wins. Threshold metrics use the same 0.5 cutoff as accuracy. See "
           "docs/evaluation_metrics.md for interpretation.")
    return pre
