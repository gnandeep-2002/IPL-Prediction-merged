...
def binary_log_loss(y, p) -> float:
    """
    Negative log-likelihood per sample, with the standard 1e-15 probability
    clip: isotonic calibration can emit exact 0.0/1.0 probabilities, and
    without clipping a single confidently-wrong prediction would send the
    metric to infinity. Implemented directly (same formula as
    src/temperature_scaling.py's NLL) so the value cannot drift with
    sklearn's changing log_loss clipping defaults.
    """
...
def classification_metrics(y, p, p_ref=None, with_ece: bool = True) -> dict:
    ...
    ECE -- plus log loss and the threshold-0.5 classification metrics
    (precision, recall, F1, confusion counts). Shared by the pre-match and
    dynamic evaluations; the pre-existing keys keep their historical order,
    new keys are appended.

    Threshold metrics use 0.5, matching the accuracy definition this
    pipeline has always reported. The positive class is whatever y==1
    means for the caller (pre-match: team1/bat-first wins; dynamic 2nd:
    the chasing side wins; dynamic 1st: the defending side wins).
    """
    y = np.asarray(y)
    p = np.asarray(p, dtype=float)
    pred = (p > 0.5).astype(int)

    # AUC is undefined when y contains a single class (roc_auc_score would
    # raise); report chance-level 0.5 for such degenerate slices so the row
    # stays complete and JSON-safe. No real split in this pipeline is
    # single-class -- this is robustness for edge slices only.
    row = {
    ...
    # Undefined ratios (empty denominator) report 0 by convention -- the
    # climatology baseline (never predicts positive) exercises this.
    precision = tp / (tp + fp) if (tp + fp) else 0.0
    recall = tp / (tp + fn) if (tp + fn) else 0.0        # a.k.a. sensitivity / TPR
    specificity = tn / (tn + fp) if (tn + fp) else 0.0   # a.k.a. TNR
...
def train_pre_match_internal(match_df: pd.DataFrame) -> dict:
    ...
    # Climatology keeps its historical convention values (AUC pinned at 0.5,
    # acc = base rate, BSS/ECE 0 by definition); the new threshold metrics
    # are computed for it like any other model (constant p ~0.48 predicts
    # the negative class every time, hence P/R/F1 of 0 -- informative as a
    # baseline, not a bug).
...
def evaluate_2026_pre_match(match_df: pd.DataFrame, pre_model, sc_pre, pm26_path: str) -> dict:
    ...
    # Full probabilistic + threshold metric row for the holdout, with
    # "bat-first side wins" as the positive class (the direction the model's
    # p_bat probability is expressed in). BSS reference: the 2008-2025
    # training base rate, i.e. the same climatology convention as the
    # internal evaluation.
